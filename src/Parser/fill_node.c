...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/14 16:05:48 by ychirouz          #+#    #+#             */
/*   Updated: 2024/02/14 16:05:50 by ychirouz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../minishell.h"

static t_ms	*mini_init(void)
{
	t_ms	*mini;
	...
	mini->full_cmd = NULL;
	mini->full_path = NULL;
	mini->rd_file = STDIN_FILENO;
	mini->wr_file = STDOUT_FILENO;
	return (mini);
}

static t_ms	*get_params(t_ms *node, char **a[2], int *i)
{
	if (a[0][*i])
	{
		if (a[0][*i][0] == '>' && a[0][*i + 1] && a[0][*i + 1][0] == '>')
			node = open_file_out_2(node, a[1], i);
		else if (a[0][*i][0] == '>')
			node = open_file_out_1(node, a[1], i);
		else if (a[0][*i][0] == '<' && a[0][*i + 1] && \
			a[0][*i + 1][0] == '<')
			node = open_file_in_2(node, a[1], i);
		else if (a[0][*i][0] == '<')
			node = open_file_in_1(node, a[1], i);
		else if (a[0][*i][0] != '|')
			node->full_cmd = ft_extend_matrix(node->full_cmd, a[1][*i]);
		else
		{
			ms_perror(PIPENDERR, NULL, 2);
			*i = -2;
		}
		return (node);
	}
